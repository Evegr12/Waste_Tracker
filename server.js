...
io.on('connection', (socket) => {
  ...
  // Evento para asociar el usuario con su socket.id
  socket.on('registrarUsuario', (usuarioId) => {
      usuariosConectados.set(usuarioId, socket.id);
      console.log(`Usuario con ID ${usuarioId} registrado con el socket ${socket.id}`);
  });

  // Manejar actualizaciones en el estado de la recolección
  socket.on('actualizarEstadoRecoleccion', async (data) => {
      const { usuarioIdRestaurante, usuarioIdRecolector, solicitudId, estado } = data;

      // Variables para mensajes específicos
      let mensajeRestaurante = '';
      let mensajeRecolector = '';
      let eta = ''; // Para la estimación de tiempo de llegada en el estado 'en camino'

      // Manejo de estados y mensajes específicos
      switch (estado) {
          case 'pendiente':
              // Mensajes para cuando la solicitud está pendiente
              mensajeRestaurante = 'Haz solicitado una recolección. Espera que sea aceptada.';
              mensajeRecolector = 'Nueva solicitud de recolección pendiente.';
              break;

          case 'en proceso':
              // Mensajes para cuando la solicitud fue aceptada
              mensajeRestaurante = 'Tu solicitud ha sido aceptada y está en proceso.';
              mensajeRecolector = 'Muy bien, aceptaste la recolección de residuos.';
              break;

          case 'en camino':
              // Mensajes para cuando el recolector va en camino
              eta = obtenerETA(); // Suponiendo que tienes una función para obtener la ETA ingresada por el recolector
              mensajeRestaurante = `El recolector está en camino. Hora estimada de llegada: ${eta}.`;
              mensajeRecolector = 'Suerte en tu viaje hacia el restaurante.';
              break;

          case 'llegada':
              // Mensajes para cuando el recolector llegó al restaurante
              mensajeRestaurante = 'El recolector ha llegado al restaurante.';
              mensajeRecolector = 'Llegaste al restaurante. Asegúrate de brindar un buen servicio.';
              break;

          case 'finalizada':
              // Mensajes para cuando la recolección fue finalizada
              mensajeRestaurante = 'La recolección ha sido finalizada. Ahora puedes calificar el servicio.';
              mensajeRecolector = '¡Bien hecho! Has finalizado la recolección.';
              break;

          default:
              console.warn('Estado no reconocido:', estado);
              return; // No realizar ninguna acción si el estado no es reconocido
      }

      // Crear notificaciones en la base de datos
      try {
          if (mensajeRestaurante) {
              await Notificacion.create({ usuarios_id: usuarioIdRestaurante, mensaje: mensajeRestaurante, solicitud_id: solicitudId });
          }
          if (mensajeRecolector) {
              await Notificacion.create({ usuarios_id: usuarioIdRecolector, mensaje: mensajeRecolector, solicitud_id: solicitudId });
          }
      } catch (error) {
          console.error('Error al crear la notificación:', error);
      }

      // Enviar notificaciones en tiempo real si los usuarios están conectados
      const socketIdRestaurante = usuariosConectados.get(usuarioIdRestaurante);
      if (socketIdRestaurante) {
          io.to(socketIdRestaurante).emit('notificacionUsuario', {
              mensaje: mensajeRestaurante,
              solicitudId: solicitudId,
              estado: estado,
              fecha: new Date().toISOString()
          });
      } else {
          console.log(`Restaurante con ID ${usuarioIdRestaurante} no está conectado.`);
      }

      const socketIdRecolector = usuariosConectados.get(usuarioIdRecolector);
      if (socketIdRecolector) {
          io.to(socketIdRecolector).emit('notificacionUsuario', {
              mensaje: mensajeRecolector,
              solicitudId: solicitudId,
              estado: estado,
              fecha: new Date().toISOString()
          });
      } else {
          console.log(`Recolector con ID ${usuarioIdRecolector} no está conectado.`);
      }

      // Emitir actualización del mapa para todos los usuarios conectados
      io.emit('actualizarMapa', { solicitudId: solicitudId, nuevoEstado: estado });
  });

  // Evento para manejar la desconexión del usuario
  socket.on('disconnect', () => {
      // Buscar y eliminar el usuario desconectado del mapa
  ...
  try {
    ...
    if (results && results.length > 0) {
      ...
      res.cookie('jwt', token, {
        httpOnly: true,  // La cookie no será accesible mediante JavaScript
